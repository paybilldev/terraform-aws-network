<p align="center">
  <a href="https://paybill.dev" target="_blank">
    <picture>
      <img src="./static/subnets.png" alt="Banner" />
    </picture>
  </a>
</p>
...
This Terraform configuration sets up **networking inside an AWS VPC**, creating both **private and public subnets** with routing, NAT gateways, Network ACLs, and outputs for integration with other modules.

### In brief — what it does:

### **Private Subnet Infrastructure**

* Creates multiple **private subnets** in a specified VPC.
* Optionally creates a **Network ACL** for private subnets with configurable ingress/egress rules.
* Creates **private route tables** and associates them to each private subnet.
* If NAT is enabled, creates **routes from private subnets to the internet via NAT Gateways** (so private resources can access the internet securely).


### **Public Subnet Infrastructure**

* Creates multiple **public subnets**.
* Creates **public route tables**, associates them with subnets.
* Adds a default route to the **Internet Gateway** (IGW) for public subnets.
* Optionally creates **Elastic IPs and NAT Gateways** — one per public subnet if enabled.
* NAT Gateway is then used by private subnets for outbound traffic.


### **Outputs**

It exports IDs, ARNs, CIDR blocks, NAT Gateway IDs, and route table IDs — making the subnets easy to consume in other modules (EC2, RDS, EKS, App services, etc.).
